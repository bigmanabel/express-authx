...
A secure and robust authentication API built with Express.js, TypeScript, and
MongoDB.

## Features

- 🔐 **JWT Authentication**: Secure authentication with access and refresh
  tokens
- 👮‍♂️ **Role-Based Access Control**: Admin and regular user roles
- ✅ **Input Validation**: Comprehensive validation using class-validator and
  DTOs
- 🛡️ **Security**: Helmet, CORS, rate limiting, and other security best
  practices
- 🗄️ **Database**: MongoDB with Mongoose for data persistence
- ⚡ **Redis**: Session management and token storage
- 📋 **Logging**: Structured logging with Winston
- 🚨 **Error Handling**: Centralized error handling with proper HTTP status
  codes
...
This will start MongoDB and Redis containers.
...
- 🔧 `PATCH /api/auth/update-password` - Update password (requires
  authentication)
...
- 🔑 `POST /api/users/api-keys` - Generate new API key (requires authentication)
- 📋 `GET /api/users/api-keys` - List user's API keys (requires authentication)
- 🗑️ `DELETE /api/users/api-keys/:id` - Delete API key (requires authentication)
...
├── models/         # Database entities (Mongoose schemas)
